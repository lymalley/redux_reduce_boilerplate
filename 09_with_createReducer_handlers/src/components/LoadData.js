...
import {fetchDrivers, fetchStages, fetchCars, deleteDrivers, deleteStages, deleteCars} from "../actions/";
...
const LoadData = ({dispatch, state}) => {
  ...
  const displayContent = (stateSlice) => {
    ...
    if (state[stateSlice].error) return <div className="error">{state[stateSlice].error}</div>;
    return state[stateSlice].data.map((c, i) => <div key={i}>{c}
      <button onClick={handleDelete(stateSlice, i)}>x</button>
    </div>)
  };
  ...
  return (
    <div className="loaded-data-container">
      ...
      <button onClick={() => {dispatch({type: 'CUSTOM_ACTION'})}}>custom car action</button>
    </div>
  );
};
